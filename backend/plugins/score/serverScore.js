...
//check break barriers
  ...
  var stats = [
    {category: "engagement", acronym: "ENG"},
    {category: "speed", acronym: "SPD"},
    {category: "accuracy", acronym: "ACC"},
    {category: "behavior", acronym: "BEH"},
    {category: "social", acronym: "SOC"}
  ];
...
//    console.log(data);
    _.each(stats, function(stat) {
      var data = this;
      if (data.hasOwnProperty(stat.category)) {
        var data = data[stat.category];
        var value = data.value;
        var max = data.max;

        var key = "CAMP:" + idSession + ':STAT:' + studentID + ':' + stat.acronym + ':'; //add date or lessonid
        var keyMax = "CAMP:" + idSession + ':STAT:' + studentID + ':' + stat.acronym + 'MAX:'; //add date or lessonid
...
//    }
    ...
    //importance
    if (!redis) {
      console.log("Redis Undefined in Server Score - Stats Add");
      return;
    }
...
//      console.log("Average =" + parseInt(replies[0]) / parseInt(replies[1]));
  ...
  function studentGetStats(socket, studentID, callback) {
    ...
    if (!redis) {
      console.log("Redis Undefined in Server Score - Get Statss");
      return;
    }
...
//  var totalScore = this.totalScore;
  ...
  this.addPoints = function(data) {
    ...
    if (!redis) {
      console.log("Redis Undefined in Server Score - student score");
      return;
    }
