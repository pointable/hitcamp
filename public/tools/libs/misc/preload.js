...
var IMGPreloader = function(options) {
  ...
  this._concurrentConnectionsPerHost = 100;   //TODO: Choose best option based on browser
};
...
IMGPreloader.prototype.removeProcessedItem = function removeProcessedItem(item, shouldReprocess) {
  ...
  var host = uri.host();
  var hostQueue = this._processingImagesByHost[host];
  if (!hostQueue){
    return;
  }
  var hostQueueLength = hostQueue.length;
