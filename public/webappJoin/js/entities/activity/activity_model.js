...
], function($, _, Backbone, StudentManager,Elements) {
...
//          return '/adventures/edit/' + this.get('idLesson') + '/activities/' + (this.isNew() ? '' : this.id +'/');
  ...
  StudentManager.reqres.setHandler("activity:entity", function(id) {
    return API.getActivityEntity(id);
  });
  
   StudentManager.reqres.setHandler("activity:entity:new", function(id) {
    return new Activity();
  });
