...
//  var startSeed = 5;
  ...
  $.widget("ryanf.wordsearchwidget", $.ui.mouse, {
    ...
    _create: function() {
      ...
      document.gameWordTaken = this.gameWordTaken;
...
//        });
    ...
    _mouseStop: function(event) {
      ...
      if (wordIndex != -1) {
        ...
        window.sendWord(wordIndex, true);
        $('.rf-glowing, .rf-highlight', this.element[0]).each(function() {
    ...
    gameWordTaken: function(index) {
      console.log("FOUND" + index);

      var selectedWord = document.model.wordList.get(index);
      $(selectedWord.cellsUsed).each(function() {
        Visualizer.select($(this.td), true);
      });

      GameWidgetHelper.signalWordFound(index);
...
//------------------------------------------------------------------------------
  ...
  var Visualizer = {
    ...
    select: function(c, isWrong) {
      $(c).removeClass("rf-armed")
          .removeClass("rf-glowing")
          .animate({'opacity': '20'}, 10, "linear", function() {
            console.log(isWrong);
            if (isWrong) {
              $(c).addClass("word-taken")

            }
            $(c).removeClass("rf-highlight").addClass("rf-selected")
                .animate({'opacity': 'show'}, 10, "linear")
          })
...
//------------------------------------------------------------------------------
  ...
  function Grid() {
    ...
    this.put = function(row, col, word) {
      ...
      var populator = eval("new " + eval("this.directions[" + Math.floor(Util.mathRandom() * 4) + "]") + "Populator(row,col,word, this)");
      ...
      for (var i = 0; i < this.size(); i++) {
        for (var j = 0; j < this.size(); j++) {
          if (this.cells[i][j].isUnwritten()) {
            this.cells[i][j].value = String.fromCharCode(Math.floor(65 + Util.mathRandom() * 26));
          }
        }
      }
  ...
  function WordList() {
    ...
    this.isWordPresent = function(word2check) {
      for (var x = 0; x < this.words.length; x++) {
        if (this.words[x].checkIfSimilar(word2check))
          return x;
      }
      return -1;
    }
